...
namespace // Local utility functions
{
	...
	void computeSmoothingNormals(tinyobj::attrib_t &attrib, const tinyobj::mesh_t &mesh,
...
static bool LoadObjAndConvert(float bmin[3], float bmax[3],
...

  {
    ...
    for (size_t s = 0; s < shapes.size(); s++) {
			...
			// For each mesh in the shape
			for (size_t meshIndex = 0; meshIndex < shapes[s].meshes.size(); ++meshIndex) {

				const tinyobj::mesh_t &mesh = shapes[s].meshes[meshIndex];
